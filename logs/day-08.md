# Day 8 - January 19, 2026

## What I Learned Today
- continue learning freeCodeCamp CSS

## What I Built/Practiced
- Flexbox and Typhography.
- Built: A Page of Playing Cards (implementing the above).

## Challenges I Faced
- 
## How I Solved Them
- 

## Resources Used
- freeCodeCamp: "CSS"

## Time Spent
- Morning: 3.
- Afternoon: 2 hours (freeCodeCamp CSS lessons)
- Evening: 
- **Total: 5 hours**

## Tomorrow's Goals
- Continue freeCodeCamp: Complete "CSS"
- exploring project
- Make my seventh daily commit

## Mood/Reflection
Need to maintain my pace; start to fell a bit slow, perhaps due to a lot of theory materials to read.

## Code Snippets Worth Remembering
form {
  display: flex;
  flex-direction: column;
  gap: 15px;
  justify-content: center, space-around, space-between
}

## What Surprised Me Today


## Questions for Tomorrow
- Where to find inspiration to start build my project :) 